---
title: "백준 3045번 변형시키기-대표적인 연결 리스트 문제(ABC 문제)"
comments: true
author-profile: true
categories: [data-structure]
---
*백준 알고리즘 3045번 문제는 정답률이 약 30%인 문제이며 연결 리스트를 다루고 있다.*
...
우선 __1단계!__ <BR/>
![이미지](https://user-images.githubusercontent.com/50163676/88367199-20755300-cdc6-11ea-9b95-6d4398701fe5.png "C 함수 알고리즘 01")<BR/>(a의 p가 가리키는 녀석의 n이 b를 가리키도록 변경한다.)<BR/><BR/>
![이미지](https://user-images.githubusercontent.com/50163676/88366410-0fc3dd80-cdc4-11ea-96d6-384b0fc35894.jpeg "C 함수 알고리즘 02")<BR/>(b의 p가 가리키는 녀석의 n이 a를 가리키도록 변경한다.)<BR/><BR/>
![이미지](https://user-images.githubusercontent.com/50163676/88366412-12263780-cdc4-11ea-92ed-1e5b75f4d118.jpeg "C 함수 알고리즘 03")<BR/>(a의 n이 가리키는 녀석의 p가 b를 가리키도록 변경한다.)<BR/><BR/>
![이미지](https://user-images.githubusercontent.com/50163676/88366414-13effb00-cdc4-11ea-9d02-0d7a64d703b5.jpeg "C 함수 알고리즘 04")<BR/>(b의 n이 가리키는 녀석의 p가 a를 가리키도록 변경한다.)<BR/><BR/>
__여기까지 변경한 후에 그림을 보기 좋게 다듬어 보자.__ <BR/>
![이미지](https://user-images.githubusercontent.com/50163676/88366418-15b9be80-cdc4-11ea-8005-01aaa0727089.jpeg "중간 단계")<BR/><BR/>
다시 시작한다. 어느덧 5단계까지 왔다. <BR/>
![이미지](https://user-images.githubusercontent.com/50163676/88366422-18b4af00-cdc4-11ea-98f8-8417f33a7da4.jpeg "C 함수 알고리즘 05")<BR/>(b의 n이 가리키는 녀석의 p가 a를 가리키도록 변경한다.)<BR/><BR/>
![이미지](https://user-images.githubusercontent.com/50163676/88366424-1a7e7280-cdc4-11ea-89bf-0d575a94adcd.jpeg "C 함수 알고리즘 06")<BR/>('a의 p가 가리키는 녀석'을 a의 n이 가리키도록 변경한다.)<BR/><BR/>
![이미지](https://user-images.githubusercontent.com/50163676/88366431-1ce0cc80-cdc4-11ea-9b4c-494b5db4f696.jpeg "C 함수 알고리즘 07")<BR/>('a의 n이 가리키는 녀석'을 b의 n이 가리키도록 변경한다.)<BR/><BR/>
![이미지](https://user-images.githubusercontent.com/50163676/88366435-1e11f980-cdc4-11ea-9dd0-cc768b526af4.jpeg "C 함수 알고리즘 08+최종")<BR/>('b의 n이 가리키는 녀석'을 a의 p가 가리키도록 변경한다.)<BR/>
__여기까지 마치면 최종 완성인 줄 착각할 수 있다. 중요한 것이 있다. 어느 코딩에서나 중요한, '예외 처리'다. 처음 1단계부터 4단계까지의 부분에서 노드 a와 b가 각각 연결 리스트 끄트머리의 노드인 경우 예외사항을 추가해 주니 문제 없이 컴파일되었다!__ 코드 화면을 참고하시라.<BR/>
지금까지 8단계 알고리즘을 사용하여 복잡한 이중 연결 리스트를 가지고 놀아 보았다. 이제 이들을 main() 함수와 함께 한데 엮으면 최종 코드가 완성된다.<BR/>

```cpp
int main() {
    int n = 0, m = 0; // 노드의 개수 N과 연산의 수 M.
    cin >> n >> m;
    
    DoubleLinkedList list; // 이 작업을 위한 이중 연결리스트 생성.
    for (int i = 0; i < n; i++) {
        list.setting(i, new Node(i + 1)); // 차례차례 이중 연결리스트를 만들어 간다.
    }
    
    while (m > 0) {
        m--;
        // 무슨 연산을, 어떤 위치의 두 노드에 대하여 실행할 것인지 입력을 받는다.
        char method; int targetNode = 0; int workNode = 0;
        cin >> method >> targetNode >> workNode;
        
        if (method == 'A') {
            Node* x = list.find(targetNode);
            //노드 X를 노드 Y의 뒤로 이동한다.
            list.remove(targetNode);
            list.insert(workNode, x);
            list.display();
            ...
            list.remove(targetNode);
            list.preInsert(workNode, x);
            list.display();
        }
        else if (method == 'C') {
            Node* x = list.find(targetNode);
            Node* y = list.find(workNode);
            list.swapNode(x, y);
            list.display();
        }
        else {
            cout << "::invalid operation. Start again::" << endl;
            break;
        }
    }
...
```
<BR/>결과창 또한 첨부한다. 코딩을 한 후 저 예제대로 입출력이 원활히 되는지 테스트하길 바란다.<BR/>
![이미지](https://user-images.githubusercontent.com/50163676/88369649-74366b00-cdcb-11ea-9b76-fff307facc77.png "백준 3045번 결과창")
<BR/>2차원 배열을 다루는 데에도 쩔쩔맸던 내가 어느덧 자라나 어려운 알고리즘 문제를 해결하는 것을 보면 절로 뿌듯해진다. 코딩 실력 향상에는 반복된 이론 복습도 한몫했지만, 학년이 올라가 학교에서 자료구조 강의를 수강하기 시작하면서 많이 나아졌던 부분이 더 크다. 마치 수학과 같다. 입으로, 눈으로만 하면 실력이 좋아질 수 없다. 좋아하는 플레이리스트를 틀고 노이즈 캔슬링 헤드폰이나 이어폰을 낀 채로 키보드를 열심히 두들기다 연습장에 그림도 그리면 어느새 작은 프로젝트 하나를 기획하는 자신을 발견할 것이다. 전국에 있는, 나와 같은 대학(원)생 개발자 지망생들에게 응원의 기운을 보내며 이만 글을 마무리짓는다.<BR/><BR/>
